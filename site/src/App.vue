<template>
  <div class="min-h-screen bg-gray-100">
            ...
            <template v-if="userStore.isLoggedIn">
              ...
              <button @click="$router.push('/notifications')" class="relative text-gray-600 hover:text-blue-500">
                ...
                <span v-if="unreadCount > 0"
                  class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{{ unreadCount > 99 ? '99+' : unreadCount }}</span>
              </button>
    ...
    <main :class="isAuthPage ? '' : 'max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6'">
      <router-view />
    </main>
  </div>
</template>
